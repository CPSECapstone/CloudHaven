...
router.get("/calendar/:user/:vendor", function (req, res) {
  eventModel.find(
    { user: req.params.user, vendor: req.params.vendor },
    function (err, eventData) {
      if (err || eventData === null) {
        res.send(
          "User " +
            req.params.user +
            " does not have any events assciated with Vendor " +
            req.params.vendor
        );
      } else {
        const userEvents = eventData.map((eventDoc) => {
...
router.post("/calendar/:user", function (req, res) {
  ...
  newEvent.save(function (err, eventData) {
    if (err || eventData === null) {
      res.send(
        "A database error occured while updating event: " + req.params.event
      );
    } else {
      res.send(eventData);
    }
...
router.delete("calendar/:event", function (req, res) {
  eventModel.findByIdAndDelete(req.params.event, function (err, eventData) {
    if (err || eventData === null) {
      res.send(
        "A database error occured while removing event: " + req.params.event
      );
    } else {
      res.send(eventData);
    }
